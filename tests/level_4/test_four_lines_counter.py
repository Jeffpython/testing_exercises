...
def test__count_lines_in__returns_number_of_lines(create_file, content, expected):
    filepath = create_file(content=content)
...
def test__count_lines_in__line_is_not_counted_if_it_starts_with_number_sign(create_file, content, expected):
    filepath = create_file(content=content)
    assert count_lines_in(filepath) == expected
